function gv_fit_dipper_Z_vs_n

close all
clear all
% GV 23/2/16
% this script takes the parameters Z and n from the fit to the mean data
% and allows either Z or n to vary separately and then calculates the mean
% RMS. This tells us whether Z or n varies more across participants
% p and q are fixed
%
% need to set either Z or n manually in three places, make sure the files
% to save is named correctly 


col = 'r';
no_of_fits = 50;
no_of_sims = 10000; % number of simulations
to_plot = 1; % 1 = show a figure for each observer; 0 = nope
delete_outliers = 1;
outlierstd = 3;
dipperlevelsdB = [-18 6 18 30]; % contrast in dB
toplot = 1;
...
K.p = K.q + ap(4);
    ...
    K.q = 2;
    K.p = 2.4; % changing these parameters to canonical values is better for
    K.Z = 5.67379229134967;
    % K.n = 0.209468653129070;
